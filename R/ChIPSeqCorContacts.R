...
#comparing ChIPseq data with the number p-scores of intersected contacts

ChIPSeqCorContacts <- function(Experiments_4C,rearranged_rawData,ChIPseqVScontacts_correlation_plots)
{
	...
	by_winds <- "no"
	winds <- "NA"
	psORreads <- "pScores" #are p-score or reads correlated with the ChIPseq data
	save_flag <- 0 #marks if there was any plot saved, if yes then the details are entered into 'ChIPseqVScontacts_correlation_plots'
	...
	#asking if to compare multiple ChIPseq files against one p-score file or one ChIPseq file against multiple p-score files
	choice1 <- as.integer(readline(prompt=cat("\nchoose how you would like to compare the files:\n1) compare one ChIPseq file against multiple p-score files\n2) compare one p-score file against multiple ChIPseq files\n\n")))
	if(choice1 == 1)
	{
	...
	else
	{
		...
		repeat
		{
			...
			else
			{
				...
				{
					#the translocation area needs to be removed both from the p-score and from the peaks file
					translocated_flag <- 1
				}
				break
			}
	...
	while(files_flag == 0)
	{
		...
		{
			cat("\nremoving translocated areas:\nthe purpose of this is in order to be able and intersect the p-scores with peaks bands\n")
			...
			#removing the sections from the peaks and ps data
			...
			for(r in seq(1,length(lin_nums2),by=2))
			{
				...
				#taking all the chromosomes that aren't getting the removal treatment, and adding them to the peaks and ps data
				...
				if(first==1)#if first is the 1st line in the file
				{
					...
					{			
						...
						else if(raw_dat[second,1]==raw_dat[(second+1),1]) #if second isn't the last of the chromosome (2)
						{
							...
							peaks_t <- peaks_afterCO[(peaks_afterCO[,1]==ch)&(peaks_afterCO[,2]>=raw_dat[(second+1),2]),]
						}
					}	
				...
				else if(raw_dat[first,1]==raw_dat[(first-1),1])#if first is in the middle of a chromosome
				{
					if(second!=nrow(raw_dat)) #if second isn't last line
					{
						if(raw_dat[second,1]!=raw_dat[(second+1),1]) #if second is the last in chromosome (6)
						{
							...
							peaks_t <- peaks_afterCO[(peaks_afterCO[,1]==ch)&(peaks_afterCO[,2]<=raw_dat[(first-1),2]),]
						}
						else if(raw_dat[second,1]==raw_dat[(second+1),1]) #if second isn't last in chromosome (5)
						{
							...
							peaks_t <- peaks_afterCO[(peaks_afterCO[,1]==ch)&((peaks_afterCO[,2]<=raw_dat[(first-1),2])|(peaks_afterCO[,2]>=raw_dat[(second+1),2])),]
						}
					}
		...
		write.table(ps_bed_afterCO,"~/Analyze4C/temp/ps_bed_afterCO.bed",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")	
		...
		{
			system("bedtools intersect -a ~/Analyze4C/temp/ps_bed_afterCO.bed -b ~/Analyze4C/temp/peaks.bed -wo > ~/Analyze4C/temp/intersected.bed")			
			...
			{
				cat("\nthere were no intersections between the p-scores and peaks data\n\n")
				return()
			}
			...
			system("bedtools intersect -a ~/Analyze4C/temp/reads_bed.bed -b ~/Analyze4C/temp/peaks.bed -wo > ~/Analyze4C/temp/intersected_reads.bed")
		...
		{	
			...
			intersected_pairs <- as.data.frame(cbind(as.numeric(ps_windows[,4]),as.numeric(peaks_windows[,4])))
			...
			intersected_reads_pairs <- as.data.frame(cbind(as.numeric(reads_windows[,4]),as.numeric(peaks_windows[,4])))			
		}
		...
		else
		{
			...
			intersected_reads_pairs <- cbind(intersected_reads_pairs,rep(peaks_datName,nrow(intersected_reads_pairs)))			
		}
		
		colnames(intersected_pairs) <- c("pScore","peaks","names")
		#intersected_pairs <- as.data.frame(intersected_pairs)
		intersected_pairs_all <- rbind(intersected_pairs_all,intersected_pairs)

		colnames(intersected_reads_pairs) <- c("reads","peaks","names")
		...
		#spearman
		cat("\nthe spearman correlation coefficient for the p-score vs. peaks:\n")
		print(cor.test(intersected_pairs$pScore,intersected_pairs$peaks,method="spearman",exact=FALSE))
		#print(cor.test(as.numeric(intersected_pairs$pScore),as.numeric(intersected_pairs$peaks),method="spearman",exact=FALSE))
		#pearson
		cat("\nthe pearson correlation coefficient for the p-score vs. peaks:\n")
		print(cor.test(intersected_pairs$pScore,intersected_pairs$peaks))
		#print(cor.test(as.numeric(intersected_pairs$pScore),as.numeric(intersected_pairs$peaks)))

		#spearman
		cat("\nthe spearman correlation coefficient for the reads vs. peaks:\n")
		print(cor.test(intersected_reads_pairs_all$reads,intersected_reads_pairs_all$peaks,method="spearman",exact=FALSE))
		#print(cor.test(as.numeric(intersected_reads_pairs_all$reads),as.numeric(intersected_reads_pairs_all$peaks),method="spearman",exact=FALSE))
		#pearson
		cat("\nthe pearson correlation coefficient for the reads vs. peaks:\n")
		print(cor.test(intersected_reads_pairs_all$reads,intersected_reads_pairs_all$peaks))
		#print(cor.test(as.numeric(intersected_reads_pairs_all$reads),as.numeric(intersected_reads_pairs_all$peaks)))

		if(choice1 == 1)
		{
			#asking if to choose another p-score file to correlate
			ans6 <- readline(prompt=cat("\nwould you like to choose another p-score file to intersect with the peaks data?\ny/n\n\n"))
			if(ans6 == "y")
			{
				z <- z + 1				
	...
	cat("\ncreating the p-score VS. peaks plot\nplease wait...\n\n")
	#print(ggplot2::ggplot(intersected_pairs_all, ggplot2::aes(x=pScore,y=peaks,color=names)) + ggplot2::scale_x_log10() +  ggplot2::scale_y_log10() + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE))
	#print(ggplot2::ggplot(intersected_pairs_all, ggplot2::aes(x=pScore,y=peaks,color=names)) + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE) + ggplot2::scale_y_continuous(limits = c(0,5000)) + ggplot2::ggtitle("contact VS. expression - correlation") + ggplot2::theme(plot.title = ggplot2::element_text(size=15,hjust = 0.5)))
	print(ggplot2::ggplot(intersected_pairs_all, ggplot2::aes(x=pScore,y=peaks,color=names)) + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE) + ggplot2::ggtitle("contact VS. ChIPseq - correlation") + ggplot2::theme(plot.title = ggplot2::element_text(size=15,hjust = 0.5)))
	...
	{
		#save the plot
		nm <- paste("ChIPseqVScontacts_correlation_plots_pScores_vs_peaks_",DandT2,".png",sep="")
	...
	#print(ggplot2::ggplot(intersected_reads_pairs_all, ggplot2::aes(x=reads,y=peaks,color=names)) + ggplot2::scale_x_log10() +  ggplot2::scale_y_log10() + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE))
	#print(ggplot2::ggplot(intersected_reads_pairs_all, ggplot2::aes(x=reads,y=peaks,color=names)) + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE) + ggplot2::scale_y_continuous(limits = c(0,5000)) + ggplot2::ggtitle("contact VS. expression - reads correlation") + ggplot2::theme(plot.title = ggplot2::element_text(size=15,hjust = 0.5)))
	print(ggplot2::ggplot(intersected_reads_pairs_all, ggplot2::aes(x=reads,y=peaks,color=names)) + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm,se=FALSE,fullrange=TRUE) + ggplot2::ggtitle("contact VS. ChIPseq - reads correlation") + ggplot2::theme(plot.title = ggplot2::element_text(size=15,hjust = 0.5)))
	...
	if(save_flag == 1)
	{
		...
		ChIPseqVScontacts_correlation_plots[nrow(ChIPseqVScontacts_correlation_plots)+1,] <- c(psORreads,num_of_pScore_files,ps_Experiments,num_of_peaks_files,peaks_Experiments,CO_ps_type,CO_ps,CO_ps_chroms,CO_source,CO_peaks_type,CO_peaks,CO_peaks_chroms,int_chroms,secs_rmv,by_winds,winds,value_source,DandT1)
