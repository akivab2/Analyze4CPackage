...
CO_compare <- function(Experiments_4C,coIntersections_plots,rearranged_rawData)
{
	...
	RE1 <- as.integer(readline(prompt=cat("\nplease enter the size of the window by RE sites (should be an even number):\n(this size is static and will be applied to all examples in this run)\n\n")))
	...
	{	
		...
		cat("\n\ncalculating p-scores for",file.name_1,"with RE windows of",RE1,"\nplease wait...\n\n")
		#using the parameters above we calculate the p score with RE site and bp window sizes
		ps_1 <- pScore(rawData_1,cis_1,vp.pos_1,erase_1,(prp_1*RE1),(RE1/2))	
	}
	else
	{
		...
		ps_1 <- pScore_nokb(rawData_1,cis_1,vp.pos_1,erase_1,RE1/2)		
	}
	
	cat("\nfor file ",file.name_2,":\n",sep="")
	#get the window size in RE site
	RE2 <- as.integer(readline(prompt=cat("\nplease enter the size of the window by RE sites (should be an even number):\n(this size is static and will be applied to all examples in this run)\n\n")))
	...
	{	
		...
		#calculating p-scores for the 1st raw data (if a translocated data is chosen this must be with no kb and before translocation areas are removed):
		cat("\n\ncalculating p-scores for",file.name_2,"with RE windows of",RE2,"\nplease wait...\n\n")
		#using the parameters above we calculate the p score with RE site and bp window sizes
		ps_2 <- pScore(rawData_2,cis_2,vp.pos_2,erase_2,(prp_2*RE2),(RE2/2))		
	}
	else
	{
		#if there are only windows of RE
		...
		cat("\n\ncalculating p-scores for",file.name_2,"with RE windows of",RE2,"\nplease wait...\n\n")		
		#using the parameters above we calculate the p score with RE site window sizes only
		ps_2 <- pScore_nokb(rawData_2,cis_2,vp.pos_2,erase_2,RE2/2)	
	}
	...
	{	
		...
		if(choice3_1 == 1)
		{
			min_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the minimum p-score cutoff that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			max_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the maximum p-score cutoff that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			step_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the p-score cutoff step that will be applied to all chromosomes:\n\n")))
			co_type1 <- "fixed"
		}
		else
		{
			min_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the minimum p-score cutoff percent (between 0 and 1) that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			max_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the maximum p-score cutoff percent (between 0 and 1) that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			step_CO1 <- as.numeric(readline(prompt=cat("\nplease enter the size of the p-score cutoff percent (between 0 and 1) step that will be applied to all chromosomes:\n\n")))
	...
	{	
		...
		if(choice3_2 == 1)
		{
			min_CO2 <- as.numeric(readline(prompt=cat("\nplease enter the size of the minimum p-score cutoff that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			max_CO2 <- as.numeric(readline(prompt=cat("\nplease enter the size of the maximum p-score cutoff that will be applied to all chromosomes. Anything above the cutoff will be considered a positive RE site:\n\n")))
			step_CO2 <- as.numeric(readline(prompt=cat("\nplease enter the size of the p-score cutoff step that will be applied to all chromosomes:\n\n")))
			co_type2 <- "fixed"
		}
		else
		{
	...
	len <- (length(seq(min_CO1,max_CO1,by=step_CO1)))*(length(seq(min_CO2,max_CO2,by=step_CO2))) #getting the size of each table
	#calculating table sizes and creating empty tables for the precisions
	nms <- c("COs1","COs2","precision","chromosome")
	...
	nms <- c("COs1","COs2","recall","chromosome")
	...
	nms <- c("COs1","COs2","Fmeasure","chromosome")
	...
	nms <- c("COs1","COs2","intersections","chromosome")
	...
	flag <- 0 #the first time we do a loop of all cutoffs for the 2nd raw data flag is 0, the second time we don't need to calculate them since we already did, so flag is 1
	flag_rmv1 <- 0
	flag_rmv2 <- 0
	ind <- 1 #the index for the sum_ints tables
	in_lns1 <- 0
	in_lns2 <- 0
	for(i in seq(min_CO1,max_CO1,by=step_CO1))
	{
		if(flag_rmv1 == 1)
		{
		...
		#applying cutoff and getting contact bands for the static data(use contactBands_byChrom_translocatedRemoval if translocation, this will first apply cutoff then remove translocations and then get contact bands):
		{
			if(choice3_1 == 1)
			{
				cat("\n\ncreating contact bands for",file.name_1,"with RE windows of",RE1,"and a cutoff of",i,"\nplease wait...\n\n")
			}
			else
			{
				cat("\n\ncreating contact bands for",file.name_1,"with RE windows of",RE1,"and a cutoff percent",i,"\nplease wait...\n\n")
			}
			...
			{	
				...
				{
					conts_out1 <- contactBands_byChrom(ps_1,RE_gap_1,bp_gap_1,i,genome_sizes,rearranged_rawData,flag_rmv1,in_lns1,1,file.name_1) #the function 'contactBands_byChrom' with the input 1 will remove the added sections after applying the cutoff and then get contact bands
				...
				{
					conts_out1 <- contactBands_byChrom(ps_1,RE_gap_1,bp_gap_1,i,genome_sizes,rearranged_rawData,flag_rmv1,in_lns1) #the function 'contactBands_byChrom' will apply the cutoff without removing any sections
					conts_1 <- conts_out1[[1]]
					in_lns1 <- conts_out1[[2]]
					if(flag_rmv1 == 0)
					{
			...
			{
				conts_1 <- contactBands(ps_1,RE_gap_1,bp_gap_1,cis_1,i,genome_sizes)
			}
			else if(choice3_1 == 2) #if the cutoff option chosen is a percentage cutoff
			{
				conts_out1 <- contactBands_byChrom(ps_1,RE_gap_1,bp_gap_1,i,genome_sizes,rearranged_rawData,flag_rmv1,in_lns1)
				conts_1 <- conts_out1[[1]]
				in_lns1 <- conts_out1[[2]]
				if(flag_rmv1 == 0)
				{
			...
			write.table(conts_1,paste("~/Analyze4C/temp/conts_1_",i,"CO_all.bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
			write.table(conts_1[conts_1[,1]!=cis_1,],paste("~/Analyze4C/temp/conts_1_",i,"CO_trans.bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
			
			for(n in 1:num_of_chroms)
			{
				write.table(conts_1[conts_1[,1]==n,],paste("~/Analyze4C/temp/conts_1_",i,"CO_",n,".bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)	
			}	
		}
		
		for(j in seq(min_CO2,max_CO2,by=step_CO2))
		{
			...
			#create the contact bands files only on the first round of for loop, erase them only at the end of everything:
			if(flag == 0)
			{	
				#creating contact bands files:
				#applying cutoff and getting contact bands for the static data(use contactBands_byChrom_translocatedRemoval if translocation, this will first apply cutoff then remove translocations and then get contact bands):
				{
					if(choice3_2 == 1)
					{
						cat("\n\ncreating contact bands for",file.name_2,"with RE windows of",RE2,"and a cutoff of",j,"\nplease wait...\n\n")
					}
					else
					{
						cat("\n\ncreating contact bands for",file.name_2,"with RE windows of",RE2,"and a cutoff percent",j,"\nplease wait...\n\n")
					}				
					#creating the contact bands by asking for cutoffs first
					...
					{	
						...
						{
							conts_out2 <- contactBands_byChrom(ps_2,RE_gap_2,bp_gap_2,j,genome_sizes,rearranged_rawData,flag_rmv2,in_lns2,1,file.name_2) #the function 'contactBands_byChrom' with the input 1 will remove the added sections after applying the cutoff and then get contact bands
						...
						{
							conts_out2 <- contactBands_byChrom(ps_2,RE_gap_2,bp_gap_2,j,genome_sizes,rearranged_rawData,flag_rmv2,in_lns2) #the function 'contactBands_byChrom' will apply the cutoff without removing any sections
							conts_2 <- conts_out2[[1]]
							in_lns2 <- conts_out2[[2]]
							if(flag_rmv2 == 0)
							{	
					...
					{
						conts_2 <- contactBands(ps_2,RE_gap_2,bp_gap_2,cis_2,j,genome_sizes)
					}
					else if(choice3_2 == 2) #if the cutoff option chosen is a percentage cutoff
					{
						conts_out2 <- contactBands_byChrom(ps_2,RE_gap_2,bp_gap_2,j,genome_sizes,rearranged_rawData,flag_rmv2,in_lns2)
						conts_2 <- conts_out2[[1]]
						in_lns2 <- conts_out2[[2]]
						if(flag_rmv2 == 0)
						{
					...
					write.table(conts_2,paste("~/Analyze4C/temp/conts_2_",j,"CO_all.bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
					write.table(conts_2[conts_2[,1]!=cis_2,],paste("~/Analyze4C/temp/conts_2_",j,"CO_trans.bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
					
					for(n in 1:num_of_chroms)
					{
						write.table(conts_2[conts_2[,1]==n,],paste("~/Analyze4C/temp/conts_2_",j,"CO_",n,".bed",sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)	
					}	
				}
			}
			...
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_all.bed -b ~/Analyze4C/temp/conts_1_",i,"CO_all.bed -wo > ~/Analyze4C/temp/conts_1_",i,"CO_all_self.bed",sep="")) #self for f-measure
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_2_",j,"CO_all.bed -b ~/Analyze4C/temp/conts_2_",j,"CO_all.bed -wo > ~/Analyze4C/temp/conts_2_",j,"CO_all_self.bed",sep="")) #self for f-measure
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_all.bed -b ~/Analyze4C/temp/conts_2_",j,"CO_all.bed -wo > ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_all.bed",sep=""))
			#in order to perform f-measure, the cis for both files must be the same
			if(cis_1 == cis_2)
			{
				fmResults_all <- system(paste("perl ~/Analyze4C/proxy/precisionRecallFmeasure.pl ~/Analyze4C/temp/conts_1_",i,"CO_all_self.bed ~/Analyze4C/temp/conts_2_",j,"CO_all_self.bed ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_all.bed ",beta,sep=""),intern=TRUE)
			...
			else
			{
				...
				if(file.info(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_all.bed",sep=""))$size != 0)
				{
					all <- read.delim(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_all.bed",sep=""),header=FALSE,quote="",stringsAsFactors=FALSE)
				}
				else
				{
			...
			sum_ints_all$COs1[ind] <- i
			sum_ints_all$COs2[ind] <- j
			sum_ints_all$intersections[ind] <- sum_all
			
			#getting precision, recall, f-measure, and sum of intersections for trans:
			#intersecting files and recording in table			
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_trans.bed -b ~/Analyze4C/temp/conts_1_",i,"CO_trans.bed -wo > ~/Analyze4C/temp/conts_1_",i,"CO_trans_self.bed",sep="")) #self for f-measure
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_2_",j,"CO_trans.bed -b ~/Analyze4C/temp/conts_2_",j,"CO_trans.bed -wo > ~/Analyze4C/temp/conts_2_",j,"CO_trans_self.bed",sep="")) #self for f-measure				
			system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_trans.bed -b ~/Analyze4C/temp/conts_2_",j,"CO_trans.bed -wo > ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_trans.bed",sep=""))
			#in order to perform f-measure, the cis for both files must be the same
			if(cis_1 == cis_2)
			{
				fmResults_trans <- system(paste("perl ~/Analyze4C/proxy/precisionRecallFmeasure.pl ~/Analyze4C/temp/conts_1_",i,"CO_trans_self.bed ~/Analyze4C/temp/conts_2_",j,"CO_trans_self.bed ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_trans.bed ",beta,sep=""),intern=TRUE)
			...
			prec_trans$COs1[ind] <- i
			prec_trans$COs2[ind] <- j
			prec_trans$precision[ind] <- prf_trans[1]
			rcll_trans$COs1[ind] <- i
			rcll_trans$COs2[ind] <- j
			rcll_trans$recall[ind] <- prf_trans[2]
			fms_trans$COs1[ind] <- i
			fms_trans$COs2[ind] <- j
			...
			{
				# i added this condition later, i think its the right thing to do but not sure			
				if(file.info(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_trans.bed",sep=""))$size != 0)
				{
					trans <- read.delim(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_trans.bed",sep=""),header=FALSE,quote="",stringsAsFactors=FALSE)
				}
				else
				{
			...
			sum_ints_trans$COs1[ind] <- i
			sum_ints_trans$COs2[ind] <- j
			...
			{
				#intersecting files and recording in table
				system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_",m,".bed -b ~/Analyze4C/temp/conts_1_",i,"CO_",m,".bed -wo > ~/Analyze4C/temp/conts_1_",i,"CO_",m,"_self.bed",sep="")) #self for f-measure
				system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_2_",j,"CO_",m,".bed -b ~/Analyze4C/temp/conts_2_",j,"CO_",m,".bed -wo > ~/Analyze4C/temp/conts_2_",j,"CO_",m,"_self.bed",sep="")) #self for f-measure
				system(paste("bedtools intersect -a ~/Analyze4C/temp/conts_1_",i,"CO_",m,".bed -b ~/Analyze4C/temp/conts_2_",j,"CO_",m,".bed -wo > ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_",m,".bed",sep=""))
				#in order to perform f-measure, the cis for both files must be the same
				if(cis_1 == cis_2)
				{
					fmResults_chroms <- system(paste("perl ~/Analyze4C/proxy/precisionRecallFmeasure.pl ~/Analyze4C/temp/conts_1_",i,"CO_",m,"_self.bed ~/Analyze4C/temp/conts_2_",j,"CO_",m,"_self.bed ~/Analyze4C/temp/ints_",i,"CO_",j,"CO_",m,".bed ",cis_1," ",beta,sep=""),intern=TRUE)
				...
				#putting the recall, precision, f-measure, and sum of intersections data in the tables for all
				prec_chroms[[m]]$COs1[ind] <- i
				prec_chroms[[m]]$COs2[ind] <- j
				prec_chroms[[m]]$precision[ind] <- prf_chroms[[m]][1]
				rcll_chroms[[m]]$COs1[ind] <- i
				rcll_chroms[[m]]$COs2[ind] <- j
				rcll_chroms[[m]]$recall[ind] <- prf_chroms[[m]][2]
				fms_chroms[[m]]$COs1[ind] <- i
				fms_chroms[[m]]$COs2[ind] <- j
				...
				{
					# i added this condition later, i think its the right thing to do but not sure			
					if(file.info(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_",m,".bed",sep=""))$size != 0)
					{
						chroms[[m]] <- read.delim(paste("~/Analyze4C/temp/ints_",i,"CO_",j,"CO_",m,".bed",sep=""),header=FALSE,quote="",stringsAsFactors=FALSE)
					}
					else
					{
				...
				sum_ints_chroms[[m]]$COs1[ind] <- i
				sum_ints_chroms[[m]]$COs2[ind] <- j
	...
	DandT2 <- gsub(":","",DandT2)	
	#add titles for all plots. and maybe need to create a file of the list of plots.add plane and legend
	#3d scatterplots:
	...
	save_flag <- 0 #if at least one of the plots is saved, then we will save the details of this run in the file coIntersections_plots.txt, save_flag will be 1 if so
	...
	if(ans1 == "y")
	{
		...
		scatterplot3d::scatterplot3d(prec_all[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. precision - all",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="precision",y.margin.add=0.5)
		...
		if(ans_save == "y")
		{
			...
			png_name <- paste("cutoffCompare_precision_all_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_precision_all_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
		...
		scatterplot3d::scatterplot3d(rcll_all[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. recall - all",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_recall_all_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_recall_all_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(fms_all[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. f-measure - all",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_Fmeasure_all_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_Fmeasure_all_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(sum_ints_all[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. sumOfIntersections - all",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="sum of intersections (bp)",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_sumOfIntersections_all_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_sumOfIntersections_all_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
	...
	if(ans2 == "y")
	{
		...
		scatterplot3d::scatterplot3d(prec_trans[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. precision - trans",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="precision",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_precision_trans_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_precision_trans_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(rcll_trans[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. recall - trans",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_recall_trans_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_recall_trans_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(fms_trans[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. f-measure - trans",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")	
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_Fmeasure_trans_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_Fmeasure_trans_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(sum_ints_trans[,1:3],pch=16,highlight.3d=TRUE,type="h",main="cutoff vs. sumOfIntersections - trans",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="sum of intersections (bp)",y.margin.add=0.5)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_sumOfIntersections_trans_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_sumOfIntersections_trans_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
	...
	for(q in 1:num_of_chroms)
	{
		if(ans3 == "y")
		{
			...
			scatterplot3d::scatterplot3d(prec_chroms[[q]][,1:3],pch=16,highlight.3d=TRUE,type="h",main=paste("cutoff vs. precision - chroms",q),xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="precision",y.margin.add=0.5)
			ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
			if(ans_save == "y")
			{
				#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
				png_name <- paste("cutoffCompare_precision_chrom",q,"_",DandT2,".png",sep="")
				#png_name <- paste("cutoffCompare_precision_chrom",q,"_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
				#if(nchar(png_name) > 255)
				#{
				#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
				#	png_name <- substring(png_name,nm_length_rmv)
			...
			scatterplot3d::scatterplot3d(rcll_chroms[[q]][,1:3],pch=16,highlight.3d=TRUE,type="h",main=paste("cutoff vs. recall - chroms",q),xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
			ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
			if(ans_save == "y")
			{
				#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
				png_name <- paste("cutoffCompare_recall_chrom",q,"_",DandT2,".png",sep="")
				#png_name <- paste("cutoffCompare_recall_chrom",q,"_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
				#if(nchar(png_name) > 255)
				#{
				#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
				#	png_name <- substring(png_name,nm_length_rmv)
			...
			scatterplot3d::scatterplot3d(fms_chroms[[q]][,1:3],pch=16,highlight.3d=TRUE,type="h",main=paste("cutoff vs. f-measure - chroms",q),xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)
			ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
			if(ans_save == "y")
			{
				#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
				png_name <- paste("cutoffCompare_Fmeasure_chrom",q,"_",DandT2,".png",sep="")
				#png_name <- paste("cutoffCompare_Fmeasure_chrom",q,"_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
				#if(nchar(png_name) > 255)
				#{
				#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
				#	png_name <- substring(png_name,nm_length_rmv)
			...
			scatterplot3d::scatterplot3d(sum_ints_chroms[[q]][,1:3],pch=16,highlight.3d=TRUE,type="h",main=paste("cutoff vs. sumOfIntersections - chroms",q),xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="sum of intersections (bp)",y.margin.add=0.5)
			ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
			if(ans_save == "y")
			{
				#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
				png_name <- paste("cutoffCompare_sumOfIntersections_chrom",q,"_",DandT2,".png",sep="")
				#png_name <- paste("cutoffCompare_sumOfIntersections_chrom",q,"_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
				#if(nchar(png_name) > 255)
				#{
				#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
				#	png_name <- substring(png_name,nm_length_rmv)
	...
	{	
		...
		scatterplot3d::scatterplot3d(prec_chroms_all[,1:3],pch=16,color=prec_colors,type="h",main="cutoff vs. precision - all chroms",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="precision",y.margin.add=0.5)
		legend("bottom", legend = levels(factor(prec_chroms_all$chromosome)),col = colors, pch = 16, inset=-0.18, xpd = TRUE, horiz = TRUE)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_precision_chromsAll_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_precision_chromsAll_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(rcll_chroms_all[,1:3],pch=16,color=rcll_colors,type="h",main="cutoff vs. recall - all chroms",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)	
		legend("bottom", legend = levels(factor(rcll_chroms_all$chromosome)),col = colors, pch = 16, inset=-0.18, xpd = TRUE, horiz = TRUE)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_recall_chromsAll_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_recall_chromsAll_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(fms_chroms_all[,1:3],pch=16,color=fms_colors,type="h",main="cutoff vs. f-measure - all chroms",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="recall",y.margin.add=0.5)		
		legend("bottom", legend = levels(factor(fms_chroms_all$chromosome)),col = colors, pch = 16, inset=-0.18, xpd = TRUE, horiz = TRUE)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_Fmeasure_chromsAll_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_Fmeasure_chromsAll_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
		...
		scatterplot3d::scatterplot3d(sum_ints_chroms_all[,1:3],pch=16,color=sum_ints_colors,type="h",main="cutoff vs. sumOfIntersections - all chroms",xlab=paste("cutoff - ",RE1," RE windows\n",file.name_1,sep=""),ylab=paste("cutoff - ",RE2," RE windows\n",file.name_2,sep=""),zlab="sum of intersections (bp)",y.margin.add=0.5)
		legend("bottom", legend = levels(factor(sum_ints_chroms_all$chromosome)),col = colors, pch = 16, inset=-0.18, xpd = TRUE, horiz = TRUE)
		ans_save <- readline("\nwould you like to save this plot?\ny/n\n\n")	
		if(ans_save == "y")
		{
			#testing to see that the name isn't longer than 256 characters (the file name can't be longer than PATH_MAX which is 256 bytes) if it is longer than we shorten it from the head of the name till it's short enough
			png_name <- paste("cutoffCompare_sumOfIntersections_chromsAll_",DandT2,".png",sep="")
			#png_name <- paste("cutoffCompare_sumOfIntersections_chromsAll_",sp1_1[1],"_vs_",sp1_2[1],"_",DandT2,".png",sep="")
			#if(nchar(png_name) > 255)
			#{
			#	nm_length_rmv <- as.integer(nchar(png_name)-255+1)
			#	png_name <- substring(png_name,nm_length_rmv)
	...
	sink(paste("~/Analyze4C/plots/cutoffCompare_maxValues_",DandT2,".txt",sep=""))
	print(list_max)
	sink()
	
	#saving the data into coIntersections_plots.txt
	if(save_flag ==  1)
	{
		#saving the parameters and details to coIntersections_plots.txt
		coIntersections_plots[nrow(coIntersections_plots)+1,] <- c(file.name_1,co_type1,min_CO1,max_CO1,step_CO1,prp_1,RE_gap_1,bp_gap_1,RE1,file.name_2,co_type2,min_CO2,max_CO2,step_CO2,prp_2,RE_gap_2,bp_gap_2,RE2,beta,DandT1)
		#sorting the list of experiments by bait alphabetically (and sorting the row indices)
		coIntersections_plots <- coIntersections_plots[order(coIntersections_plots$filename_1),]
		rownames(coIntersections_plots) <- seq(length=nrow(coIntersections_plots))
		#adding the new data to the file (by erasing the old one and creating a new one)
		system("rm coIntersections_plots.txt")
